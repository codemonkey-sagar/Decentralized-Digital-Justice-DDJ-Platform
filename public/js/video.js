...
function bufferToHex(buffer) {
    console.log("buffer hex invoked.");
...
function createRecord(blob) {
    return blob
        ...
        .then((hashBuffer) => {
            let hashed_value = bufferToHex(hashBuffer);
            try {
                fetch("/video/storehash", {
                    method: "POST",
                    headers: {
...
function downloadContent(videoBlob) {
    ...
    var questionsBlob = new Blob([JSON.stringify(shuffledQuestions, null, 2)], {
...
downloadButton.addEventListener(
    ...
    function () {
        ...
        fetch(blobUrl)
            ...
            .catch(function (error) {
                return console.error("Error downloading video:", error);
            });
    },
    false
);
